        ...
        <>
          <div className={`balance-card ${animateBalance ? "animate" : ""}`}>
            ...
            <div className="balance-details">
              <span>💰 Disponible: {formatCOPWithPrivacy(availableBalance)}</span>
              <span>🏦 En bolsillos: {formatCOPWithPrivacy(totalInPockets)}</span>
            </div>
            {totalCardDebt > 0 && <div className="card-debt-warning">💳 Deuda tarjetas: {formatCOPWithPrivacy(totalCardDebt)}</div>}
            {totalPendingReceivables > 0 && <div className="receivable-info">💰 Por cobrar: {formatCOPWithPrivacy(totalPendingReceivables)}</div>}
            <div className="net-worth">💰 Patrimonio neto: {formatCOPWithPrivacy(netWorth)}</div>
          </div>
                      ...
                      <div key={card.id} className="card-mini-card">
                        <div className="card-mini-header">
                          ...
                          <span className={`card-usage ${usagePercent > 80 ? "danger" : usagePercent > 50 ? "warning" : ""}`}>
                            {formatCOPWithPrivacy(card.used_amount)} / {formatCOPWithPrivacy(card.limit_amount)}
                          </span>
                        </div>
                        ...
                        <div className="card-mini-footer">
                          <span>Disponible: {formatCOPWithPrivacy(card.available_limit)}</span>
                    ...
                    return (
                      <div key={budget.id} className="budget-progress-item">
                        <div className="budget-header">
                          ...
                          <span>{formatCOPWithPrivacy(budget.collected)} / {formatCOPWithPrivacy(budget.amount)}</span>
                        </div>
                        <div className="progress-bar">
                          <div className="progress-fill" style={{ width: `${Math.min(progress, 100)}%`, background: progress > 90 ? "#e74c3c" : "#2ecc71" }}></div>
                        </div>
                        <div className="budget-remaining">Restante: {formatCOPWithPrivacy(budget.amount - budget.collected)}</div>
                      </div>
                    );
                  })}
                </div>
                    ...
                    return (
                      <div key={item.id} className="receivable-progress-item">
                        <div className="receivable-header">
                          ...
                          <span>{formatCOPWithPrivacy(item.collected)} / {formatCOPWithPrivacy(item.amount)}</span>
                        </div>
                        <div className="progress-bar">
                          <div className="progress-fill receivable-fill" style={{ width: `${Math.min(progress, 100)}%` }}></div>
                        </div>
                        <div className="receivable-remaining">Por cobrar: {formatCOPWithPrivacy(item.amount - item.collected)}</div>
                      </div>
                    );
                  })}
                </div>
                  ...
                  <div className="pockets-grid">
                    {pockets.map((pocket) => (
                      <div key={pocket.id} className="pocket-mini-card">
                        ...
                        <span className="pocket-amount">{formatCOPWithPrivacy(pocket.current)}</span>
                        {pocket.goal && (
            ...
            {listTransactions.length === 0 ? (
              ...
              listTransactions.slice(0, 10).map((t) => (
                <div key={t.id} className={`transaction ${t.type}`} onClick={() => {
                  ...
                  <span className="amount">{t.type === "gasto" ? "-" : "+"}{formatCOPWithPrivacy(t.amount)}</span>
                ...
                <div key={card.id} className="card-detail">
                  <div className="card-header">
                    ...
                    <div className="card-actions">
                      {card.used_amount > 0 && (
                        <button onClick={() => payCard(card.id)} className="pay-card-btn">💰 Pagar {formatCOPWithPrivacy(card.used_amount)}</button>
                      )}
                      <button onClick={() => {
                        setSelectedCardForAdvance(card);
                        setShowAdvanceModal(true);
                  ...
                  <div className="card-stats">
                    <div className="stat"><span>Cupo total:</span><strong>{formatCOPWithPrivacy(card.limit_amount)}</strong></div>
                    <div className="stat"><span>Usado:</span><strong className="used">{formatCOPWithPrivacy(card.used_amount)}</strong></div>
                    <div className="stat"><span>Disponible:</span><strong className="available">{formatCOPWithPrivacy(card.available_limit)}</strong></div>
                  </div>
                  <div className="progress-bar">
                    <div className="progress-fill" style={{ width: `${Math.min(usagePercent, 100)}%`, background: usagePercent > 80 ? "#e74c3c" : usagePercent > 50 ? "#f39c12" : "#2ecc71" }}></div>
                  </div>
                    ...
                    <div className="card-transactions-list">
                      ...
                      {cardTransactions.filter((t) => t.card_id === card.id).map((t) => (
                        <div key={t.id} className="card-transaction-item" onClick={() => {
                          ...
                          <span className="card-tx-amount">{formatCOPWithPrivacy(t.amount)}</span>
                ...
                <div key={item.id} className="budget-card">
                  <div className="budget-card-header">
                    ...
                    <div className="budget-stats">
                      <span className="budget-spent">{formatCOPWithPrivacy(item.collected)}</span>
                      <span className="budget-divider">/</span>
                      <span className="budget-total">{formatCOPWithPrivacy(item.amount)}</span>
                    </div>
                  </div>
                  <div className="progress-bar">
                  ...
                  <div className={`budget-remaining ${remaining < 0 ? "negative" : ""}`}>
                    {progress >= 100 ? "✅ Completado" : `📊 Restante: ${formatCOPWithPrivacy(remaining)} (${Math.round(progress)}% usado)`}
                  </div>
                      ...
                      <button onClick={async () => {
                        if (window.confirm(`¿Pagar los ${formatCOP(remaining)} restantes de ${item.name}?`)) {
                          ...
                          const created = await createTransaction({
                            user_id: userId,
                            list_id: activeList,
                            description: `💰 Pago completo: ${item.name}`,
                            amount: numAmount,
                            type: "gasto",
                          ...
                          alert(`✅ Pagado ${formatCOP(numAmount)} de ${item.name}`);
                        }
                ...
                <div key={item.id} className="receivable-card">
                  <div className="receivable-card-header">
                    ...
                    <div className="receivable-stats">
                      <span className="receivable-collected">{formatCOPWithPrivacy(item.collected)}</span>
                      <span className="receivable-divider">/</span>
                      <span className="receivable-total">{formatCOPWithPrivacy(item.amount)}</span>
                    </div>
                  </div>
                  <div className="progress-bar">
                    <div className="progress-fill receivable-fill" style={{ width: `${Math.min(progress, 100)}%` }}></div>
                  ...
                  <div className="receivable-remaining">
                    {progress >= 100 ? "✅ Completado" : `💰 Por cobrar: ${formatCOPWithPrivacy(remaining)} (${Math.round(progress)}% cobrado)`}
                  </div>
                      ...
                      <button onClick={async () => {
                        if (window.confirm(`¿Registrar cobro completo de ${formatCOP(remaining)} de ${item.name}?`)) {
                          ...
                          const created = await createTransaction({
                            user_id: userId,
                            list_id: activeList,
                            description: `💰 Cobro completo: ${item.name}`,
                            amount: numAmount,
                            type: "ingreso",
                            payment_method: "efectivo",
                          ...
                          alert(`✅ Cobrado ${formatCOP(numAmount)} de ${item.name}`);
                        }
      ...
      {activeView === "pockets" && (
        <div className="pockets-view">
          ...
          <div className="balance-info">
            <div className="info-card"><span>💰 Balance disponible:</span><strong>{formatCOPWithPrivacy(availableBalance)}</strong></div>
          </div>
                ...
                <div key={pocket.id} className="pocket-card">
                  <div className="pocket-header">
                    <div className="pocket-title">
                      ...
                      <div><h4>{pocket.name}</h4>{pocket.goal && <small>Meta: {formatCOPWithPrivacy(pocket.goal)}</small>}</div>
                    </div>
                  ...
                  <div className="pocket-balance">
                    ...
                    <span className="balance-value">{formatCOPWithPrivacy(pocket.current)}</span>
                  </div>
                  {pocket.goal && (
                    <>
                      <div className="progress-bar"><div className="progress-fill" style={{ width: `${Math.min(progress, 100)}%` }}></div></div>
                      <div className="pocket-goal-info">{progress >= 100 ? "🎉 ¡Meta alcanzada!" : `Falta ${formatCOPWithPrivacy(pocket.goal - pocket.current)}`}</div>
                    </>
                  )}
                </div>
              );
            ...
            <form onSubmit={handleAddTransaction}>
              ...
              {paymentMethod === "tarjeta" && transactionType === "gasto" && (
                <select value={selectedCard} onChange={(e) => setSelectedCard(e.target.value)} required>
                  ...
                  {creditCards.map((c) => <option key={c.id} value={c.id}>{c.icon} {c.name} (Disponible: {formatCOPWithPrivacy(c.available_limit)})</option>)}
                </select>
              )}
              ...
              {transactionType === "gasto" && paymentMethod === "efectivo" && pockets.length > 0 && (
                <select value={selectedPocket} onChange={(e) => setSelectedPocket(e.target.value)}>
                  ...
                  {pockets.map((p) => <option key={p.id} value={p.id}>{p.icon} {p.name} ({formatCOPWithPrivacy(p.current)})</option>)}
                </select>
              )}
          ...
          <div className="modal-content">
            ...
            <form onSubmit={handleAddPartialExpense}>
              ...
              <div className="budget-info-modal">
                <div className="info-row"><span>Presupuesto total:</span><strong>{formatCOPWithPrivacy(selectedFixedItem.amount)}</strong></div>
                <div className="info-row"><span>Gastado:</span><strong>{formatCOPWithPrivacy(selectedFixedItem.collected)}</strong></div>
                <div className="info-row highlight"><span>Disponible:</span><strong>{formatCOPWithPrivacy(selectedFixedItem.amount - selectedFixedItem.collected)}</strong></div>
              </div>
              <div className="modal-buttons">
        ...
        <div className="modal">
          <div className="modal-content">
            ...
            <form onSubmit={handleAddPartialIncome}>
              ...
              <div className="budget-info-modal">
                <div className="info-row"><span>Total por cobrar:</span><strong>{formatCOPWithPrivacy(selectedReceivable.amount)}</strong></div>
                <div className="info-row"><span>Cobrado:</span><strong>{formatCOPWithPrivacy(selectedReceivable.collected)}</strong></div>
                <div className="info-row highlight"><span>Pendiente:</span><strong>{formatCOPWithPrivacy(selectedReceivable.amount - selectedReceivable.collected)}</strong></div>
              </div>
              <div className="modal-buttons">
        ...
        <div className="modal">
          <div className="modal-content">
            ...
            <form onSubmit={(e) => { e.preventDefault(); executeTransfer(); }}>
              ...
              <div className="balance-info-text">Disponible: {formatCOPWithPrivacy(availableBalance)}</div>
              <div className="modal-buttons">
        ...
        <div className="modal" onClick={() => {
          ...
          <div className="modal-content" onClick={(e) => e.stopPropagation()}>
            ...
            <p className="advance-card-info">
              ...
              Cupo disponible: <strong className="available">{formatCOPWithPrivacy(selectedCardForAdvance.available_limit)}</strong>
            </p>
